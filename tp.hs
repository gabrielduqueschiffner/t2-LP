...
-----------------------------------------------------------------------------------------------------------------------------------
-- Função para marcar uma posição como Bombada
 --marcarBombas :: Int -> Coordenada -> Matriz -> I0 ()
 --marcarBombas coordenada matriz = do
 -- mapM_ (imprimirColuna n) [0..n-1]
 -- where
 --   imprimirColuna :: Int -> Int -> IO ()
 --   imprimirColuna n coluna = do
 ...
 --   imprimirCelulaBombada :: Int -> Int -> I0 ()
  --  imprimirCelulaBombada coluna linha 
  --       | isMarcadoBomba matriz coordenada == True = putStrLn "A posição já foi marcada como Bombada" 
  --       | otherwise = putStr "B"


  --  isMarcadoBomba :: Matriz -> Coordenada -> Bool
  --  isMarcadoBomba matriz (linha, coluna) = isMarcadoBomba matriz (linha, coluna) && (linha, coluna) /= (linha, coluna)

--FUncao auxiliar para guardar o estado do campo minado


-----------------------------------------------------------------------------------------------------------------------------------

--funcao para converter string int
...
-- putStrLn "Informe a posição (linha e coluna) que deseja abrir:"
 --jogada <- getLin
--   if validCoord matriz coordenada
 --       then do
 --         if isBomba == True
  --          then do
  --            putStrLn "Boom! Você perdeu!"
   --         else do
   --           let matrizAtualizada = updateElement matrizAtualizada posicao
   --           jogar matrizAtualizada
   --     else do
